...
class TorqueTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for r in TorqueTest.g.cog.scanner():
            print(r.key, "==>", ', '.join(r.value))
    ...
    def test_torque_22(self):
        ...
        TorqueTest.g = Graph(graph_name="love_graph", cog_home=DIR_NAME)
        TorqueTest.g.put("A", "love", "B")\
            .put("B", "love", "C")\
            .put("A", "love", "D")\
            .put("Z", "love", "D")\
            .put("D", "love", "F")\
        ...
        expected = {'result': [{'id': 'D', 'from': 'A', 'to': 'D'}, {'id': 'B', 'from': 'A', 'to': 'B'}]}
        actual = TorqueTest.g.v("A").tag("from").out("love").tag("to").all()
        print("Before deletion Graph(A):",actual)
        self.assertTrue(actual == expected)
        
        # After deletion
        TorqueTest.g.delete("A", "love", "D")
        after_deletion_expected = {'result': [{'id': 'B', 'from': 'A', 'to': 'B'}]}
        after_deletion_actual = TorqueTest.g.v("A").tag("from").out("love").tag("to").all()
        print("After deletion Graph(A): ",after_deletion_actual)
        self.assertTrue(after_deletion_actual == after_deletion_expected)
