...
class Cog:
    ...
    def use_table(self, name):
        ...
        if name not in self.namespaces[self.current_namespace] or self.namespaces[self.current_namespace][name] is not Table:
    ...
    def update_set(self, data):
        # update value of an existing node
        assert type(data.key) is str, "Only string type is supported."
        assert type(data.value) is str, "Only string type is supported."
        record = self.current_table.indexer.get(data.key, self.current_table.store)
        if record is not None:
            if type(record.value) is str and type(data.value) is str:
                record.value = data.value
            if type(record.value) is list:
                if type(data.value) is str:
                    record.value = list(data.value)
                if type(data.value) is list:
                    record.value = data.value
            position = self.current_table.store.save(record)
            self.current_table.indexer.put(record.key, position, self.current_table.store)
    ...
    def delete_edge(self, record, predicate, vertex):
        # Delete vertex from record's value: the record's edge from/to vertex

        assert type(record) is Record, "Only Record type is supported."
        assert type(predicate) is str and predicate != "", "Only not empty string type is supported."
        assert type(vertex) is str and vertex != "", "Only not empty string type is supported."

        predicate_hashed = hash_predicate(predicate)
        self.use_table(predicate_hashed)

        if record is not None:
            self.delete_from_vertex_values(record, vertex)
            position = self.current_table.store.save(record)
            self.current_table.indexer.put(record.key, position, self.current_table.store)

    def delete_from_vertex_values(self, record, vertex):
        if record is not None:
            if type(record.value) is str and record.value == vertex:
                #cancella completamente nodo
                record.value 
            if type(record.value) is list and vertex in record.value:
                if len(record.value) == 1:
                    #cancella completamente nodo
                    record.value = ""
                else:
                    record.value = lists_diff(record.value, list(vertex))
