...
class Graph:
    ...
    def delete(self, vertex1, predicate="", vertex2=""):
        ...
        assert type(vertex1) is str and vertex1 != "", "Only not empty string type is supported."
        assert type(predicate) is str and predicate != "", "Only not empty string type is supported."
        assert type(vertex2) is str and vertex2 != "", "Only not empty string type is supported."

        predicate_hashed = hash_predicate(predicate)
        self.cog.use_table(predicate_hashed)

        v1_outnodes = out_nodes(vertex1)
        v2_innodes = in_nodes(vertex2)

        for r in self.cog.scanner():
            if r.key == v1_outnodes and vertex2 in r.value:
                self.cog.delete_edge(r, predicate, vertex2)

            if r.key == v2_innodes and vertex1 in r.value:
                self.cog.delete_edge(r, predicate, vertex1)
